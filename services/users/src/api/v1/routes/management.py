...
from libs.auth_lib.utils import MGMT_ROLE_CHECKER, ROLES
...
@router.get(
    "/users", response_model=UserPublic, dependencies=[Depends(MGMT_ROLE_CHECKER)]
)
...
@router.get("/roles", dependencies=[Depends(MGMT_ROLE_CHECKER)])
...
@router.patch(
    "/users/{user_id}/role",
    dependencies=[Depends(MGMT_ROLE_CHECKER)],
...
async def update_role(
    ...
    user = await update_user_role(
        session=session, user_id=user_id, role=new_role, commit=False
    )
