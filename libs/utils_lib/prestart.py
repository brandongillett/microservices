...
from libs.utils_lib.core.faststream import NatsClient
...
async def db_create_database(session_manager: DatabaseSessionManager) -> None:
    """
...
async def nats_create_stream(nats: NatsClient) -> None:
    """
    Creates a NATS stream if it does not exist.

    Args:
        nats (NatsClient): The NATS client instance.
    """
    try:
...
async def test_nats_connection(nats: NatsClient) -> None:
    """
    Tests the NATS connection.
